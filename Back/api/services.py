...
#   IN_PROGRESS -> RESOLVED (assigned IT or superadmin only)
#   RESOLVED -> DONE (requester or superadmin only)
#   RESOLVED -> IN_PROGRESS (requester can reject the resolution)
TRANSITIONS = {
    Ticket.Status.NEW: {Ticket.Status.IN_PROGRESS},
    Ticket.Status.IN_PROGRESS: {Ticket.Status.RESOLVED},
    Ticket.Status.RESOLVED: {Ticket.Status.DONE, Ticket.Status.IN_PROGRESS},
}
...
def notify(user, title, message, ntype, ticket=None, related_request=None):
    if not user:
        return
...
def apply_status(ticket, new_status, actor):
    ...
    elif new_status == Ticket.Status.RESOLVED and old == Ticket.Status.IN_PROGRESS:
        ensure_assigned_technician()
        ticket.resolved_at = timezone.now()
        log_history(ticket, actor, "Solution proposée", old, new_status)
    elif new_status == Ticket.Status.DONE and old == Ticket.Status.RESOLVED:
    ...
    elif new_status == Ticket.Status.IN_PROGRESS and old == Ticket.Status.RESOLVED:
        if not is_owner:
            raise PermissionDenied("Seul le demandeur peut refuser la solution.")
        log_history(ticket, actor, "Solution refusée", old, new_status)
    else:
