...
class NotificationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = (
            "id", "title", "message", "type", "related_ticket", "related_request",
...
class AttendanceSerializer(serializers.ModelSerializer):
    ...
    def validate_date(self, value):
        if value.year < 2000 or value.year > 2100:
            raise serializers.ValidationError("La date doit être comprise entre 2000 et 2100.")
        return value
...
class EmployeeRequestSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if kind == EmployeeRequest.Kind.LEAVE:
            ...
            if attrs["amount"] <= 0:
                raise serializers.ValidationError({"amount": "Le montant doit être supérieur à zéro."})
            attrs.setdefault("destination", EmployeeRequest.Destination.HR)
